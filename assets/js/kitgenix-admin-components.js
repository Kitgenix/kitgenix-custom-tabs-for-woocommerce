    ...
    try {
      ...
      var markFailed = function () {
        try {
          ...
          var failLabel = btn.getAttribute('data-kitgenix-copy-fail-label') || 'Copy failed';
          btn.classList.add('kitgenix-copy-failed');
          btn.textContent = failLabel;
          setTimeout(function () {
            try {
              btn.classList.remove('kitgenix-copy-failed');
              btn.textContent = btn.getAttribute('data-kitgenix-copy-label') || failLabel;
            } catch (_eRevert) {}
          }, 1500);
        } catch (_eMark) {}
      };
      ...
      if (result && typeof result.then === 'function') {
        // Success and failure must show different feedback – a clipboard
        // permission denial or insecure-context rejection previously showed
        // the same "Copied!" label as an actual success.
        result.then(markCopied, markFailed);
      } else {
        markCopied();
      }
    } catch (_e) {}
