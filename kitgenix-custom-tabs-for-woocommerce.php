<?php
/**
 * Plugin Name:       Kitgenix Custom Tabs for WooCommerce
...
if ( ! function_exists( 'kitgenix_custom_tabs_for_woocommerce_register_admin_ui_style' ) ) {
	function kitgenix_custom_tabs_for_woocommerce_register_admin_ui_style(): void {
		if ( ! is_admin() ) {
			return;
		}
...
add_action( 'admin_enqueue_scripts', 'kitgenix_custom_tabs_for_woocommerce_register_admin_ui_style', 5 );
...
if ( ! defined( 'KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_DIR' ) ) {
	define( 'KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_DIR', plugin_dir_path( __FILE__ ) );
}
if ( ! defined( 'KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_URL' ) ) {
	define( 'KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_URL', plugin_dir_url( __FILE__ ) );
}
...
if ( ! function_exists( 'kitgenix_custom_tabs_for_woocommerce_ensure_admin_menu' ) ) {
	function kitgenix_custom_tabs_for_woocommerce_ensure_admin_menu(): void {
		if ( ! is_admin() ) {
			return;
		}
		...
		add_menu_page(
			__( 'Kitgenix', 'kitgenix-custom-tabs-for-woocommerce' ),
			__( 'Kitgenix', 'kitgenix-custom-tabs-for-woocommerce' ),
			$capability,
			$slug,
...
add_action( 'admin_menu', 'kitgenix_custom_tabs_for_woocommerce_ensure_admin_menu', 5 );

if ( ! function_exists( 'kitgenix_custom_tabs_for_woocommerce_enqueue_hub_styles' ) ) {
	function kitgenix_custom_tabs_for_woocommerce_enqueue_hub_styles( string $hook_suffix ): void {
...
add_action( 'admin_enqueue_scripts', 'kitgenix_custom_tabs_for_woocommerce_enqueue_hub_styles', 20 );
...
// If another Kitgenix plugin already registered the hub renderer, do not redeclare.
if ( ! function_exists( 'kitgenix_render_admin_page' ) ) {
	function kitgenix_render_admin_page(): void {
		...
		if ( ! $allowed ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to access this page.', 'kitgenix-custom-tabs-for-woocommerce' ) );
		}
		...
		$plugins = [
			[
				'id'       => 'turnstile',
				'name'     => __( 'CAPTCHA for Cloudflare Turnstile', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Add Cloudflare Turnstile CAPTCHA to WordPress, WooCommerce, Elementor, and popular form plugins.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'custom_tabs',
				'name'     => __( 'Custom Tabs for WooCommerce', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Add custom WooCommerce product tabs with per-product content, global tabs, and lightweight controls.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'documents',
				'name'     => __( 'Document Manager', 'kitgenix-custom-tabs-for-woocommerce' ),
				'slug'     => 'kitgenix-document-manager',
				'file'     => 'kitgenix-document-manager/kitgenix-document-manager.php',
				'page'     => 'kitgenix-document-manager',
				'requires' => __( 'Manage document downloads with stable links, version history, and private file access.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'tracking',
				'name'     => __( 'Order Tracking for WooCommerce', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Add WooCommerce order tracking, multi-shipment support, email tracking links, and a public customer tracking page.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'pdf',
				'name'     => __( 'PDF Invoicing for WooCommerce', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Generate WooCommerce PDF invoices, receipts, packing slips, and credit notes with secure downloads and configurable email attachments.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'stock',
				'name'     => __( 'Stock Sync for WooCommerce', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Sync WooCommerce stock between stores with secure master-child inventory updates and signed REST requests.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
			[
				'id'       => 'multistore',
			...
			[
				'id'       => 'affiliate',
				'name'     => __( 'Affiliate Link Manager', 'kitgenix-custom-tabs-for-woocommerce' ),
				...
				'requires' => __( 'Manage affiliate short links, branded redirects, and click tracking from one WordPress dashboard.', 'kitgenix-custom-tabs-for-woocommerce' ),
			],
		];
		...
		foreach ( $plugins as $p ) {
			...
			if ( $slug && ! empty( $wporg_active_installs[ $slug ] ) ) {
				...
				$installs_badge = '<span class="kitgenix-badge muted">' . esc_html( sprintf( __( '%s+ installs', 'kitgenix-custom-tabs-for-woocommerce' ), $count_text ) ) . '</span>';
			}
			...
			if ( $slug && isset( $wporg_ratings[ $slug ] ) && (int) $wporg_ratings[ $slug ] > 0 ) {
				...
				$rating_badge = '<span class="kitgenix-badge muted">' . esc_html( sprintf( __( '★ %s/5', 'kitgenix-custom-tabs-for-woocommerce' ), $stars_text ) ) . '</span>';
			}

			$status_badge = '';
			if ( ! $installed ) {
				$status_badge = '<span class="kitgenix-badge muted">' . esc_html__( 'Not installed', 'kitgenix-custom-tabs-for-woocommerce' ) . '</span>';
			} elseif ( $active ) {
				$status_badge = '<span class="kitgenix-badge ok">' . esc_html__( 'Active', 'kitgenix-custom-tabs-for-woocommerce' ) . '</span>';
			} else {
				$status_badge = '<span class="kitgenix-badge warn">' . esc_html__( 'Installed (Inactive)', 'kitgenix-custom-tabs-for-woocommerce' ) . '</span>';
			}
			...
			if ( ! $installed ) {
				...
				$actions .= '<a class="button button-primary" href="' . esc_url( $install_url ) . '">' . esc_html__( 'Install', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
			} elseif ( ! $active ) {
				if ( current_user_can( 'activate_plugins' ) ) {
					...
					$actions .= '<a class="button button-primary" href="' . esc_url( $activate_url ) . '">' . esc_html__( 'Activate', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
				} else {
					$actions .= '<span class="description">' . esc_html__( 'You do not have permission to activate plugins.', 'kitgenix-custom-tabs-for-woocommerce' ) . '</span>';
				}
			} else {
				$open_url = ! empty( $p['page'] ) ? admin_url( 'admin.php?page=' . rawurlencode( (string) $p['page'] ) ) : '';
				if ( $open_url ) {
					$actions .= '<a class="button button-primary" href="' . esc_url( $open_url ) . '">' . esc_html__( 'Open', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
				}
			}
			...
			$actions  .= ' <a class="button button-secondary thickbox open-plugin-details-modal" href="' . esc_url( $info_url ) . '">' . esc_html__( 'Details', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
			if ( $slug ) {
				...
				$actions    .= ' <a class="button button-secondary" href="' . esc_url( $review_url ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Review', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
				$actions    .= ' <a class="button button-secondary" href="' . esc_url( $support_url ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Support Forum', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
			}
			...
			echo '<div class="kitgenix-card" data-kitgenix-plugin="' . esc_attr( sanitize_key( $id ) ) . '">'
				...
				. '<div class="kitgenix-card-badges">' . wp_kses( trim( $status_badge . ' ' . $version_badge . ' ' . $rating_badge . ' ' . $installs_badge ), $allowed_html ) . '</div>'
				. '<p class="kitgenix-card-title">' . esc_html( (string) $p['name'] ) . '</p>'
				. '<p class="kitgenix-card-desc">' . esc_html( (string) $p['requires'] ) . '</p>'
				. '</div>'
				. '<div class="kitgenix-card-actions">' . wp_kses( $actions, $allowed_html ) . '</div>'
				. '</div>';
		}
...
register_activation_hook(
	...
	static function (): void {
		if ( ! class_exists( 'WooCommerce' ) ) {
			...
			wp_die( esc_html__( 'Kitgenix Custom Tabs for WooCommerce requires WooCommerce to be installed and active. The plugin has been deactivated.', 'kitgenix-custom-tabs-for-woocommerce' ) );
		}

		set_transient( 'kitgenix_custom_tabs_for_woocommerce_do_activation_redirect', 1, 30 );
	}
);

/**
 * Perform activation redirect once.
 */
add_action(
	'admin_init',
	static function (): void {
		if ( ! get_transient( 'kitgenix_custom_tabs_for_woocommerce_do_activation_redirect' ) ) {
			return;
		}
		delete_transient( 'kitgenix_custom_tabs_for_woocommerce_do_activation_redirect' );
		...
		$target = admin_url( 'admin.php?page=kitgenix-custom-tabs-for-woocommerce' );
...
add_filter(
	...
	static function ( array $links ): array {
		$url     = admin_url( 'admin.php?page=kitgenix-custom-tabs-for-woocommerce' );
		$links[] = '<a href="' . esc_url( $url ) . '">' . esc_html__( 'Settings', 'kitgenix-custom-tabs-for-woocommerce' ) . '</a>';
		return $links;
	}
);
...
spl_autoload_register(
	static function ( string $class ): void {
		$prefix = 'KitgenixCustomTabsForWooCommerce\\';
		...
		$file     = trailingslashit( KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_DIR ) . 'includes/' . $relative . '.php';
	...
	static function (): void {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility(
				'custom_order_tables',
				KITGENIX_CUSTOM_TABS_FOR_WOOCOMMERCE_FILE,
				true
			);
		}
	}
...
/**
 * Bootstrap plugin once WooCommerce is available.
 */
add_action(
	'plugins_loaded',
	static function (): void {
